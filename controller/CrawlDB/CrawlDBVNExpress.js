...
var crawler_db = new Crawler().configure({depth: 2}).crawl("https://vnexpress.net", function onSuccess(page) {
            var url =  page.url;
            request (url,(err,res,body)=>{
                if(!err && res.statusCode == 200)
                {
                    ...
                    var NoiDung = $('.sidebar_1 article').text();
                    ...
                    _BaiBao.Hinh = $('.tplCaption img').prop("src");
                    _BaiBao.Link = url;
                    var TheLoai = $('.start h4 a').text();
                    _BaiBao.TheLoai = TheLoai.replace(/\n|  /,'');
                    // _BaiBao.ThoiGian = new Date();
                    // console.log(_BaiBao.TheLoai)
                    var time = $('.sidebar_1 header span').text();
                    // console.log(time);
                    var mang = time.split(' ');
                    for(var i in mang)
                    {                        
                        ...
                        {
                            // console.log(mang[2])
                            var tmp = mang[2].split('/');
                            // console.log(tmp)
                        }
                        if(i==3)
                        {
                            var tmpTime = mang[3].split(':');
                            var tmpTimeNumber = 7+Number(tmpTime[0])
                            // console.log(tmpTimeNumber,'....................',tmpTime,'//////',tmp[2].split(','))
                            var yy=tmp[2].split(',');
                            _BaiBao.ThoiGian = new Date(yy[0],tmp[1]-1,tmp[0],tmpTimeNumber,tmpTime[1]);
                            console.log(_BaiBao.ThoiGian)
                        }
                        // console.log(mang[3])
                    }
                   
                    // _BaiBao.ThoiGian = mydate;
                    _BaiBao.save((err, Crawl_DB)=>{
                        ...
                        console.log('loi crawl khi luu gia tri!');
                            ...
                            console.log('crawl thành công!',Crawl_DB._id);
                        }
                    })
                }
                else{
